...
schema
  ...
  .is()
  .not()
  .oneOf(["Passw0rd", "Password123", "Admin@123", "Password@123"]); // Blacklist these values
...
export default function formValidator(e) {
  ...
  switch (name) {
    ...
    case "password":
      if (!value || value.length === 0) return "Password Field is Mendatory";
    ...
    case "basePrice":
      if (!value || value.length === 0) return name + " Field is Mendatory";
      else if (value < 1) return "Base Price Must Be More Than 0";
      else return "";
    ...
    case "stockQuantity":
      if (!value || value.length === 0) return name + " Field is Mendatory";
      else if (value < 0) return "Stock Quantity Must Not Be Nagative";
      else return "";
